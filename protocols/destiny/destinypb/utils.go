...
func WrapMessageInner(msg proto.Message) isDestinyMessage_Type {
	switch t := msg.(type) {
	case *NormalMessage:
		return &DestinyMessage_Normal{Normal: t}
	case *ONormalMessage:
		return &DestinyMessage_ONormal{ONormal: t}
	case *ResultMessage:
		return &DestinyMessage_Result{Result: t}
	default:
		panic(fmt.Sprintf("proto: unexpected type %T in WrapMessageInner", t))
	}
}

// WrapDestinyMessage wraps a union type of Message in a new PBFTMessage without a
// destination.
func WrapDestinyMessage(msg proto.Message) *DestinyMessage {
	return &DestinyMessage{Type: WrapMessageInner(msg)}
}
