...
func WrapMessageInner(msg proto.Message) isDQSBFTSlowMessage_Type {
	switch t := msg.(type) {
	case *NormalMessage:
		return &DQSBFTSlowMessage_Normal{Normal: t}
	case *ONormalMessage:
		return &DQSBFTSlowMessage_ONormal{ONormal: t}
	case *ResultMessage:
		return &DQSBFTSlowMessage_Result{Result: t}
	default:
		panic(fmt.Sprintf("proto: unexpected type %T in WrapMessageInner", t))
	}
}

// WrapDQSBFTSlowMessage wraps a union type of Message in a new PBFTMessage without a
// destination.
func WrapSBFTxMessage(msg proto.Message) *DQSBFTSlowMessage {
	return &DQSBFTSlowMessage{Type: WrapMessageInner(msg)}
}
