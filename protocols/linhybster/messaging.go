...
func (s *linhybster) enqueueForSending(to peerpb.PeerID, mm []byte, certificate []byte) {
	cm := peerpb.Message{
		Content:     mm,
...
func (s *linhybster) marshall(m proto.Message) []byte {
	dMsg := pb.WrapLinHybsterMessage(m)
...
func (s *linhybster) sendTo(to peerpb.PeerID, mBytes []byte, sign []byte) {
...
func (s *linhybster) broadcast(mBytes []byte, sendToSelf bool, sign []byte) {
