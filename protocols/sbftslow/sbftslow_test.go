...
func (s *sbftslow) ReadMessages() []peerpb.Message {
	msgs := s.msgs
	s.ClearMsgs()
	return msgs
}

func (s *sbftslow) ExecutableCommands() []peer.ExecPacket {
	cmds := s.executedCmds
...
type network struct {
	peers       map[peerpb.PeerID]*sbftslow
	failures    map[*sbftslow]struct{}
...
func newNetwork(nodeCount int, fastFailCount int32) network {
	peers := make(map[peerpb.PeerID]*sbftslow, nodeCount)
	...
	for _, r := range peersSlice {
		...
		logger.EnableDebug()
		peers[r] = NewSBFTSlow(&peer.LocalConfig{
			PeerConfig: &peerpb.PeerConfig{
	...
	return network{
		peers:    peers,
		failures: make(map[*sbftslow]struct{}, nodeCount),
...
func (n *network) restart(id peerpb.PeerID) {
	p := n.peers[id]
	n.peers[id] = NewSBFTSlow(&peer.LocalConfig{
		ID:       p.id,
		Peers:    p.nodes,
		RandSeed: int64(id),
	}).(*sbftslow)
...
func (n *network) crash(id peerpb.PeerID) *sbftslow {
	p := n.peers[id]
	n.failures[p] = struct{}{}
	return p
}
...
func (n *network) alive(p *sbftslow) bool {
...
func (n *network) count(pred func(*sbftslow) bool) int {
	count := 0
	for _, p := range n.peers {
		if pred(p) {
			count++
...
func (n *network) quorumHas(pred func(*sbftslow) bool) bool {
	return n.quorum(n.count(pred))
}

func (n *network) allHave(pred func(*sbftslow) bool) bool {
...
func (n *network) runNetwork(goal func(p *sbftslow) bool, quorum bool) bool {
	waitUntil := n.allHave
	if quorum {
...
func (n *network) waitExecuteInstance(inst *instance, quorum bool) bool {
	return n.runNetwork(func(p *sbftslow) bool {
